...
def permute_edge_explain_predict_loss(trial = 2):
    '''
    Parameters
    ----------
    trial : TYPE, int
    ...
    if opt.fold:
        ...
        for index1, index2 in zip(kf.split(hc_target_idx), kf.split(ad_data)):
            ...
            for data1, data2 in zip(hc_loader, patient_loader):
                ...
                loss_gt = MSELoss()(torch.exp(y_pred_dis[~data2.pcd[:,2].isnan()]).squeeze(), 
                    data2.pcd[~data2.pcd[:,2].isnan(),2]).squeeze().item() 
                # y_true_dis = data2.pcd[~data2.pcd[:,2].isnan(),2].detach().cpu().tolist()
                # y_pred_dis = torch.exp(y_pred_dis).detach().cpu().tolist()
                # r2 = r2_score(y_true_dis, y_pred_dis)
            ...
            for data1_, data2_ in zip(hc_loader, patient_loader):
                ...
                for j in range(len(idx[0])):
                    ...
                    edge2 = dense_to_sparse(edge2)
                    x, mask = to_dense_batch(data2.x, data2.batch)
                    x[:,idx[0][j],idx[1][j]] = x[idx_permute,idx[0][j],idx[1][j]]
                    x[:,idx[0][j],idx[1][j]] = x[idx_permute,idx[0][j],idx[1][j]]
                    x = x[mask]
                    output_dis, h_dis, y_pred_dis, logits_dis, output_dis_share, h_dis_share, y_pred_dis_share, logits_dis_share, \
                        output_hc, h_hc, y_pred_hc, logits_hc = model(data1.x, data1.edge_index, data1.edge_attr, data1.pcd, 
                    ...
                    loss_permute = MSELoss()(torch.exp(y_pred_dis[~data2.pcd[:,2].isnan()]).squeeze(), 
                        data2.pcd[~data2.pcd[:,2].isnan(),2]).squeeze().item() 
                    loss_onerun.append(loss_permute)
            loss_all_permute.append(loss_onerun)
    return np.array(loss_all_permute), loss_gt
...
def gcn_node_explain_captum(trial = 2, method = 'IntegratedGradients'):
    '''
    Parameters
    ----------
    trial : TYPE, int
    ...
    if opt.fold:
        ...
        for index1, index2 in zip(kf.split(hc_target_idx), kf.split(ad_data)):
            ...
            with torch.no_grad():
                for data1_, data2_ in zip(hc_loader, patient_loader):
                    a,idx = to_dense_batch(data2_.x, data2_.batch)
                    diag = [torch.diag(arr).numpy() for arr in a]
                    diag = torch.Tensor(diag)
                    diag = torch.unsqueeze(diag, -1)
                    a = a - diag
                    data2_.x = a[idx]
                    data1 = data1_.to(device)
                    data2 = data2_.to(device)
                    ...
                    data2.x = data2.x - data2.x[0,0]
                    disoder_net = model.disorder_net
                    hc_net = model.HC_net
                    explainer_dis = Explainer(
                        model=disoder_net,
                    ...
                    if method == 'IntegratedGradients' or method == 'ShapleyValueSampling':
                        explanation_dis = explainer_dis(data2.x, data2.edge_index, target = None, edge_attr = data2.edge_attr, pcd = data2.pcd, eyes = data2.eyes, 
                                                batch = data2.batch)
                        explanation_hc = explainer_hc(data1.x, data1.edge_index, target = None, edge_attr = data1.edge_attr, pcd = data1.pcd, eyes = data1.eyes, 
                                                batch = data1.batch)
    
                        # print(explanation_dis.edge_mask)
                        adj = to_dense_adj(data1.edge_index, data1.batch, explanation_dis.edge_mask).cpu().detach().numpy()
                        adj = (adj + np.transpose(adj, (0,2,1)))/2
                        node_importance_edge = adj.sum(0).sum(0)
    
                        # print(explanation_dis.node_mask)
                        node_importance_reshape, _ = to_dense_batch(explanation_dis.node_mask, data1.batch)
                        node_importance = explanation_dis.node_mask.sum(dim=0).cpu().numpy()
                        
                        adj_hc = to_dense_adj(data1.edge_index, data1.batch, explanation_hc.edge_mask).cpu().detach().numpy()
                        adj_hc = (adj_hc + np.transpose(adj_hc, (0,2,1)))/2
                        node_importance_edge_hc = adj_hc.sum(0).sum(0)
    
                        # print(explanation_dis.node_mask)
                        node_importance_reshape_hc, _ = to_dense_batch(explanation_hc.node_mask, data1.batch)
                        node_importance_hc = explanation_hc.node_mask.sum(dim=0).cpu().numpy()
                        # explanation_dis.visualize_graph('/home/alex/project/CGCN/ptau_code/result/ptau/A-T-N-/ensemb_non_15%_dyn_infomax_pretrain3/visulaiztion/GCN_captum_integratedGradient_node/with_hc/0.6/subgraph.png', backend="graphviz")
                        sio.savemat('/home/alex/project/CGCN/ptau_code/result/ptau/A-T-N-/ensemb_non_15%_dyn_infomax_pretrain3/visulaiztion/GCN_captum_node/with_hc/0.7/{}/importance_fold{}_trial_{}.mat'.format(method, i, trial), {'edge_importantce_dis': adj, 
                                          'node_importance_fromedge_dis': node_importance_edge, 'node_importance_dis': node_importance, 
                                          'node_importance_raw_dis': explanation_dis.node_mask.cpu().numpy(), 
                                          'node_importance_raw_reshape_dis': node_importance_reshape.cpu().numpy(),
                                          'edge_importantce_hc': adj_hc, 'node_importance_fromedge_hc': node_importance_edge_hc, 
                                          'node_importance_hc': node_importance_hc, 
                                          'node_importance_raw_hc': explanation_hc.node_mask.cpu().numpy(), 
                                          'node_importance_raw_reshape_hc': node_importance_reshape_hc.cpu().numpy()})
                    
                    elif method == 'Saliency' or  method == 'InputXGradient' or method == 'GuidedBackprop':
                        explainer_dis = Explainer(
                            model=disoder_net,
                            algorithm=CaptumExplainer(method),
                            explanation_type='model',
                        ...
                        explanation_dis = explainer_dis(data2.x, data2.edge_index, edge_attr = data2.edge_attr, pcd = data2.pcd, eyes = data2.eyes, 
                                                batch = data2.batch)
                        explanation_hc = explainer_hc(data1.x, data1.edge_index, edge_attr = data1.edge_attr, pcd = data1.pcd, eyes = data1.eyes, 
                                                batch = data1.batch)

                        node_importance_reshape, _ = to_dense_batch(explanation_dis.node_mask, data1.batch)
                        node_importance = explanation_dis.node_mask.sum(dim=0).cpu().numpy()
                        sio.savemat('/home/alex/project/CGCN/ptau_code/result/ptau/A-T-N-/ensemb_non_15%_dyn_infomax_pretrain3/visulaiztion/GCN_captum_node/with_hc/0.7/{}/importance_fold{}_trial_{}.mat'.format(method, i, trial), {'node_importance': node_importance, 
                                     'node_importance_raw': explanation_dis.node_mask.cpu().numpy(),
                                     'node_importance_raw_reshape': node_importance_reshape.cpu().numpy()})
                    path = '/home/alex/project/CGCN/ptau_code/result/ptau/A-T-N-/ensemb_non_15%_dyn_infomax_pretrain3/visulaiztion/GCN_captum_node/with_hc/0.7/{}/feature_importance_dis_fold{}_trial_{}.png'.format(method, i, trial)
                    explanation_dis.visualize_feature_importance(path, top_k=20)
                    path = '/home/alex/project/CGCN/ptau_code/result/ptau/A-T-N-/ensemb_non_15%_dyn_infomax_pretrain3/visulaiztion/GCN_captum_node/with_hc/0.7/{}/feature_importance_hc_fold{}_trial_{}.png'.format(method, i, trial)
                    explanation_hc.visualize_feature_importance(path, top_k=20)
                    # explanation_dis.visualize_graph('/home/alex/project/CGCN/ptau_code/result/ptau/A-T-N-/ensemb_non_15%_dyn_infomax_pretrain3/visulaiztion/GCN_captum_integratedGradient_node/with_hc/0.6/subgraph.png', backend="graphviz")
...
if __name__ == '__main__':
    ...
    # r_p_pcd_all = np.array(r_p_pcd_all)
    # sio.savemat('/home/alex/project/CGCN/A+/final_figure/ab/statistics_withPCDassociation_withrace_DementiaA+.mat', {'dis_mean_final_stats': v_p_dis_f, 'hc_mean_final_stats': v_p_hc_f,
    #           'keys_all': keys, 'dis_all_final_stats': v_p_dis_array, 'hc_all_final_stats': v_p_hc_array, 'y_dis_ten_trials': y_dis_mean_all, 
    #           'y_hc_ten_trials': y_hc_mean_all, 'dis_latent_all': dis_latent_all, 'hc_latent_all': hc_latent_all, 'dis_latent_features': dis_latent_all,
    #           'hc_latent_features': hc_latent_all, 'hc_mean_final_stats_rsa': v_p_hc_f_rsa, 'dis_mean_final_stats_rsa': v_p_dis_f_rsa, 
    #           'hc_notarget_stats': v_p_hc_f_, 'dis_notarget_stats': v_p_dis_f_, 'hc_notarget_stats_rsa': v_p_hc_f_rsa_, 'dis_notarget_stats_rsa':
    #               v_p_dis_f_rsa_, 'keys_notarget': keys_fdr, 'r_p_pcd': r_p_pcd_all})
    # pcd_all_used.to_csv('/home/alex/project/CGCN/A+/final_figure/ab/pcd_feature_withrace_DementiaA+.csv')
    ...
    #     pcd_mri = dataset.data.pcd[:len(dataset)//(opt.augmentation+1),:][dataset.data.pcd[:len(dataset)//(opt.augmentation+1),-1]==0].numpy()
    #     # pcd_all['dx'] = dx_all
    #     # pcd_all.to_csv('/home/alex/project/CGCN/A+/final_figure/ab/pcd_all_info.csv')
    #     sub, IA, IB = np.intersect1d(pcd_mri[:,0], pcd_all['subjectID'], return_indices=True)
    #     _, IAA, IBB = np.unique(IA, return_index=True, return_inverse=True)
    #     pcd_all_used = pcd_all.iloc[IB].iloc[IAA].reset_index(drop=True)
    #     dx_used = dx_all[IB][IAA]
    ...
    #         patient_loader = DataLoader(HC_data,batch_size=opt.batchSize,shuffle = False)
    #         model.eval()
    #         for data in patient_loader:
    #             data = data.to(device)
    #             disoder_net = model.disorder_net
    ...
    # # v_p_dis_f_rsa_ = np.delete(v_p_dis_f_rsa, [7,8,9],0)
    # # keys_fdr = np.delete(np.array(keys), [7,8,9],0)
    # v_p_hc_f_ = np.delete(v_p_hc_f, [7,8],0) #for MCI A-
    # v_p_dis_f_ = np.delete(v_p_dis_f, [7,8],0)
    # v_p_hc_f_rsa_ = np.delete(v_p_hc_f_rsa, [7,8],0)
    # v_p_dis_f_rsa_ = np.delete(v_p_dis_f_rsa, [7,8],0)
    # keys_fdr = np.delete(np.array(keys), [7,8],0)
    # ########## association of averaged model outcomes
    # r_p_pcd_all = []
    # for j in range(pcd_all_used.shape[-1]):
    #     mask = ~pcd_all_used.iloc[:,j].isnull()
    ...
    # r_p_pcd_all = np.array(r_p_pcd_all)
    # sio.savemat('/home/alex/project/CGCN/A+/final_figure/ab/nocontrast_model/statistics_withPCDassociation_withrace_NCA+.mat', {'dis_mean_final_stats': v_p_dis_f, 'hc_mean_final_stats': v_p_hc_f,
    #           'keys_all': keys, 'dis_all_final_stats': v_p_dis_array, 'hc_all_final_stats': v_p_hc_array, 'y_dis_ten_trials': y_dis_mean_all, 
    #           'y_hc_ten_trials': y_hc_mean_all, 'dis_latent_all': dis_latent_all, 'hc_latent_all': hc_latent_all, 'dis_latent_features': dis_latent_all,
    #           'hc_latent_features': hc_latent_all, 'hc_mean_final_stats_rsa': v_p_hc_f_rsa, 'dis_mean_final_stats_rsa': v_p_dis_f_rsa, 
    #           'hc_notarget_stats': v_p_hc_f_, 'dis_notarget_stats': v_p_dis_f_, 'hc_notarget_stats_rsa': v_p_hc_f_rsa_, 'dis_notarget_stats_rsa':
    #               v_p_dis_f_rsa_, 'keys_notarget': keys_fdr, 'r_p_pcd': r_p_pcd_all})
    # pcd_all_used.to_csv('/home/alex/project/CGCN/A+/final_figure/ab/nocontrast_model/pcd_feature_withrace_NCA+.csv')
